...
<?php else:?>
    ...
    <div class="container">
        <?php foreach ($data as $key => $kamar): ?> 
        <div class="card mb-3">
        <img src="<?=$kamar['info_kamar']->img_room?>" class="card-img-top" alt="..."style="max-height: 500px;object-fit: cover;">
        <div class="card-body">
            <h5 class="card-title"><?=$kamar['info_kamar']->Nama_room?></h5>
            <p class="card-text">
            Fasilitas:
          ...
          <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                  <a href="<?= base_url('Tamu/Formpemesanan').'?id_kamar='.$kamar['info_kamar']->id_kamar?>" class="btn btn-primary">Pesan sekarang</a>
                </div>
              </div>
            </div>
          </div>
